...
// FinanceCard defined BEFORE VehicleFinance so it can be used inside
...
const VehicleFinance = () => {
  ...
  const localizedFinance = filterByLocation(vehicleFinance, userLocation?.city);

  const filteredFinance = activeFilter === 'All' 
    ? localizedFinance 
    : localizedFinance.filter(f => f.category === activeFilter);
